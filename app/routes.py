...
def edit_alarm(alarm_id):
    ...
    if request.method == "GET":
        ...
        form.puzzle_type.data = normalize_alarm_puzzle_type(alarm.puzzle_type)
    ...
    if form.validate_on_submit():
        ...
        try:
            alarm.device_serial = device.serial_number
            alarm.time = alarm_time
            alarm.puzzle_type = normalize_alarm_puzzle_type(form.puzzle_type.data)
            alarm.use_dynamic_alarm = bool(form.use_dynamic_alarm.data)
            alarm.dynamic_start_time = (
                dynamic_start_time if form.use_dynamic_alarm.data else None
            )
                    ...
                    result[day].append(
                        {
                            ...
                            "puzzle_type": normalize_alarm_puzzle_type(
                                getattr(alarm, "puzzle_type", "recommended")
                            ),
                            "use_dynamic_alarm": bool(
...
def api_create_alarm():
    ...
    puzzle_type = normalize_alarm_puzzle_type(data.get("puzzle_type", "recommended"))
