...
AUTO_PUZZLE_TYPE_ALIASES = ("recommended", "random")
...
def normalize_alarm_puzzle_type(puzzle_type) -> str:
    """Normalize stored alarm puzzle mode names while keeping old aliases working."""
    normalized = (puzzle_type or "").strip().lower()
    if normalized in AUTO_PUZZLE_TYPE_ALIASES or not normalized:
        return "recommended"
    return normalized
...
class Alarm(db.Model):
    ...
    def create(
        ...
        alarm.puzzle_type = normalize_alarm_puzzle_type(puzzle_type)
...
def resolve_effective_puzzle_type(alarm: Alarm, device: Device = None) -> str:
    ...
    """
    stored_type = normalize_alarm_puzzle_type(getattr(alarm, "puzzle_type", ""))
