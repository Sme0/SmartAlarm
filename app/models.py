...
def _as_utc(value: datetime | None) -> datetime | None:
    """
    Normalize datetimes read from the DB so both naive and aware values can be
    compared safely. Naive values are treated as UTC because this app stores all
    server timestamps in UTC.
    """
    if value is None:
        return None
...
class Device(db.Model):
    ...
    def is_online(self) -> bool:
        last_seen = _as_utc(self.last_seen)
        if not last_seen:
            return False
        return _utc_now() - last_seen < timedelta(minutes=2)
...
class AlarmSession(db.Model):
    ...
    def create(
        ...
        session.triggered_at = _as_utc(triggered_at) or _utc_now()
